...
export async function scrapeWorkday({
  ...
  function isSummaryLocation(s) {
    const t = cleanText(s).toLowerCase();
    // Workday often prepends a "3 Standorte" / "9 Locations" counter.
    if (/^\d+\s+(standorte|locations?)$/.test(t)) return true;
    return false;
  }
  ...
  function extractLocations(listItem, detail) {
    ...
    const finalLocs = dedupeKeepOrder(locs).filter((x) => !isSummaryLocation(x));
    return {
      locations: finalLocs,
  ...
  while (jobs.length < max) {
    ...
    for (const jp of page.jobPostings) {
      ...
      // Keep a single primary location for the main column.
      // Preserve additional locations separately.
      const { locations, primary } = extractLocations(jp, detail);
      const location = primary || null;
      const locationsJoined = locations.length ? locations.join(" | ") : location;
      ...
      const workplaceRaw = extractWorkplaceRaw(jp, info, locationsJoined, descriptionText);
      const workplace = normalizeWorkplace(workplaceRaw || locationsJoined || "");
